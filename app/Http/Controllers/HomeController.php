...
class HomeController extends Controller
{
    ...
    public function search_news(Request $request){
        $output = '';
        $search_news = News::where('news_title', 'LIKE', '%'. $request->src_key .'%')->where('status', 1)->take(6)->get();
        if(count($search_news)>0){
            ...
            foreach ($search_news as $news) {
                ...
                $output .= '<li><a href="'.route('search_result').'?q='.$request->src_key.'">
                                <img src="'.asset('upload/images/thumb_img/'. $news->image->source_path).'" alt="" width="80">
                                <span class="post-content">
                                    <ul class="post-tags">
                                        <li><i class="fa fa-clock-o"></i>২১ ডিসেম্বর, ২০১৯</li>
                                    </ul>
                                </span>
                            </a> <a href="'.route('news_details', $news->news_slug).'">'.str_limit($news->news_title, 80).'</a></li>';
            }
    ...
    public function search_result(Request $request){
        ...
        $search_results = News::where('news_title', 'LIKE', '%'. $request->q .'%')->where('status', '=', 1)->paginate(20);
        // dd($search_results );
        if(Session::get('locale')){ $folder = 'frontend.en.'; }else{  $folder = 'frontend.'; }
        if($search_results){
