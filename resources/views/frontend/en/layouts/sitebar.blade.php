...
$sidebar_news=  DB::table('news')
        ->join('categories', 'news.category', '=', 'categories.id')
        ->leftJoin('sub_categories', 'news.subcategory', '=', 'sub_categories.id')
        ->leftJoin('media_galleries', 'news.thumb_image', '=', 'media_galleries.id')
        ->where('news.status', 1)
        ->where('news.lang', '=', 2)
        ->select('news.*','categories.category_en', 'sub_categories.subcategory_en','media_galleries.source_path')
                ...
                <ul class="list-posts">
                    ...
                    <li>
                        ...
                        <div class="post-content">
                            <h2><a href="{{route('news_details', $recent->news_slug)}}">{{str_limit($recent->news_title, 60)}}</a></h2>
                            <ul class="post-tags">
                ...
                <ul class="list-posts">
                    @foreach($popular_news as $popular)
                        <li>
                            ...
                            @else @endif
                            <div class="post-content">
                                <h2><a href="{{route('news_details', $popular->news_slug)}}">{{str_limit($popular->news_title, 60)}}</a></h2>
                                <ul class="post-tags">
                        ...
                        <div class="col-md-12">
                            <div class="item news-post standard-post">
                                <div class="post-gallery">
                                    <img src="{{ asset('upload/images/thumb_img_box/'. $recipe->source_path)}}" alt="">
                                    @if($recipe->type == 3)
                                        <a class="play-link" href="{{route('news_details', $recipe->news_slug)}}"><i class="fa fa-play-circle-o"></i></a>
                                    @elseif($recipe->type == 4)
                                        <a class="play-link" href="{{route('news_details', $recipe->news_slug)}}"><i class="fa fa-headphones" aria-hidden="true"></i></a>
                                    @else @endif
                                </div>

                                <div class="post-content">
                                    <h2><a href="{{route('news_details', $recipe->news_slug)}}">{{str_limit($recipe->news_title, 80)}}</a></h2>
                                </div>
                            </div>
                        </div>
                    @else
                        <div class="col-md-6 col-xs-6">
                            <div class="item news-post standard-post">
                                <div class="post-gallery">
                                    <img src="{{ asset('upload/images/thumb_img/'. $recipe->source_path)}}" alt="">
                                    @if($recipe->type == 3)
                                        <a class="play-link" href="{{route('news_details', $recipe->news_slug)}}"><i class="fa fa-play-circle-o"></i></a>
                                    @elseif($recipe->type == 4)
                                        <a class="play-link" href="{{route('news_details', $recipe->news_slug)}}"><i class="fa fa-headphones" aria-hidden="true"></i></a>
                                ...
                                <div class="post-content">
                                    <h2><a href="{{route('news_details', $recipe->news_slug)}}">{{str_limit($recipe->news_title, 50)}}</a></h2>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <li>
                        <img src="{{ asset('upload/images/thumb_img/'. $most_views->source_path)}}" alt="">
                         @if($most_views->type == 3)
                                ...
                                @else @endif
                        <div class="post-content">
                            <h2><a href="{{route('news_details', $most_views->news_slug)}}">{{str_limit($most_views->news_title, 60)}}</a></h2>
                            <ul class="post-tags">
